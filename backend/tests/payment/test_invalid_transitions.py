...
@pytest.mark.asyncio
async def test_cannot_skip_states_from_registered_to_paid():
    """Test that cannot skip PAYMENT_PENDING state."""
    ...
    # Try to confirm payment directly - should fail
...
@pytest.mark.asyncio
async def test_cannot_initiate_payment_when_already_paid():
    """Test that cannot initiate payment when already PAID."""
    exam_repo = InMemoryExamRepository()
    user_repo = InMemoryUserRepository()
    reg_repo = InMemoryRegistrationRepository()
    service = PaymentService(reg_repo, exam_repo, user_repo)
